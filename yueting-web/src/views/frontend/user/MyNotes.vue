<!-- @/views/frontend/user/MyNotes.vue -->
<template>
			...
			<div class="main-content">
				...
				<div class="left-content">
					...
					<div class="notes-list">
						<div v-if="myNotes.length === 0" class="empty-tip">还没有发布笔记</div>
						<div v-for="note in myNotes" :key="note.noteId" class="note-card">
							<!-- 用户信息 -->
							<div class="note-user">
							...
							<!-- 笔记图片列表 -->
							<div v-if="note.imageUrls && note.imageUrls.length > 0" class="note-images">
								<div v-for="(imgUrl, imgIndex) in note.imageUrls" :key="imgIndex" class="note-image-item">
									<img :src="imgUrl" />
								</div>
							</div>
							<!-- 转发的源笔记卡片 -->
				...
				<div class="right-content">
					<div class="following-card">
						<div class="card-title-row">
							<div class="card-title">我关注的用户</div>
						</div>
						<div class="following-grid">
							<div v-if="followingUsers.length === 0" class="empty-following">还没有关注任何用户</div>
							<div v-for="user in followingUsers" :key="user.userId" class="following-item" @click="goToUserHome(user.userId)">
								<img :src="user.avatarUrl || defaultAvatar" class="following-avatar" />
...
<script setup>
	...
	import { apiGetMyNotes, apiCreateNote, apiDeleteNote, apiLikeNote, apiUnlikeNote, apiCommentNote, apiDeleteComment } from '@/api/frontend/note';
	...
	const loadMyNotes = async () => {
		try {
			...
			if (res.data.code === 1) {
				...
				authStore.user.noteCount = myNotes.value.length;
			}
		} catch (error) {
	...
	const loadFollowingUsers = async () => {
		try {
			...
			if (res.data.code === 1) {
				...
				authStore.user.followingCount = followingUsers.value.length;
			}
		} catch (error) {
	...
	// 为特定用户发笔记弹框
	const openNoteDialogForUser = (user) => {
		if (!authStore.isLoggedIn) {
			router.push('/login');
			return;
		}
